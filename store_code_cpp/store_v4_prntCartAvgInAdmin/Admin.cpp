...
void Admin::setCartAvg(){
    ...
    itemAvg.setCartIndx(0, item0Purchd); 
    itemAvg.setCartIndx(1, item1Purchd); 
    itemAvg.setCartIndx(2, item2Purchd); 
    
    totalPurchd = item0Purchd + item1Purchd + item2Purchd;
    itemAvg.setCartSize(totalPurchd);
}
...
void Admin::printCartAvg(){
    ...
    for(int i=0; i < NUMQQ; i++){ 
        ...
        //float totalPurchd = itemAvg.getCartSize();
        //avg = (itemAvg.getCartIndx(i)/totalPurchd)*100;
        
        cout << " purchased: "<< itemAvg.getCartIndx(i) << " times. ";//<< setw(3) << avg << "%  ";
        getChart(itemAvg.getCartIndx(i));
    }    
    cout << "\n\t  Total items every purchased:  " << itemAvg.getCartSize() << endl;
}
