    ...
    do {
        ...
        switch(choice){
            ...
            {    
                editCart();
                readBin_setArray();  // Reset user array with records read in from latest binary file
                printCartAvg();     // Print survey results
                pause();
                break;
            }
            case 5:  
...
//
/******************************************************************/ 
void Admin::setCartAvg(){
    
    //cout << "\n\tHit setCartAvg().\n";  
    
    // Reset arrays to zero
    for(int i=0; i < NUMQQ; i++){ 
        cartAvg[i].setCartArr(0,0,0);
    }
    
    // Loop through every record in usrArr[]  
    for(int i=0; i < totalRec; i++){      
     
        //usrArr[i]->getPrntCart();        
        ...
        for(int voteIndx=0; voteIndx < NUMQQ; voteIndx++){           
            ...
            // set this person's vote for item 1, 2, and 3
            int ans = usrArr[i]->user.cart.getCartIndx(voteIndx); //  vote[0] = ques1Answer           
                                                                //  vote[1] = ques2Answer 
                                                                //  vote[2] = ques3Answer 
            ...
            if(voteIndx == count) {                  
                ...
                if( ans == 1){          // vote[count] = 1 ? QSum[0].vote[0] = +1                   
                    ...
                    //cout << " Hit ans == 1\n";
                    cartAvg[voteIndx].increCart(0); 
                
                } else if(ans == 2){  //  vote[count] = 2 ? QSum[0].vote[1] = +1
                    
                    //cout << " Hit ans == 2\n";
                    cartAvg[voteIndx].increCart(1);
                
                } else if(ans == 3){   //  vote[count] = 3 ? QSum[0].vote[2] = +1
                    
                    //cout << " Hit ans == 3\n";
                    cartAvg[voteIndx].increCart(2);
                }
                else {
                    cout<<"";
                }
...
//
/******************************************************************/ 
void Admin::printCartAvg(){
    
    cout<<"\n\t*****Survey Results*****";
    
    // Loop through usrArr[] & print their cartAvg[] 
    cout << endl << endl;
    for(int i=0; i < totalRec; i++){   
        usrArr[i]->getPrntCart();    
    }
    ...
    cout << "   Q1 total cart:" << totalRec << endl;
    cout << "   Q2 total cart: " << setw(1) << " " << totalRec << endl;
    cout << "   Q3 total cart: " << setw(3) << " " << totalRec << endl;
    ...
    for(int i=0; i < NUMQQ; i++){ 
        ...
        cout << "\n\n\t  Item " << i+1  << " results\n";
        //cout << setw(4) << " " << survey.item[i].getItem(0) << endl << endl;
             
        avg = (cartAvg[i].getCartIndx(0)/tRecFloat)*100;
        //cout << setw(18) << survey.item[i].getItem(1) << "  "  
             //<< cartAvg[i].getCartIndx(0) << " vote(s) " << setw(3) << avg << "%  ";
        getChart(cartAvg[i].getCartIndx(0));
        
        avg = (cartAvg[i].getCartIndx(1)/tRecFloat)*100;
        //cout << setw(18) << survey.item[i].getItem(2) << "  " 
             //<< cartAvg[i].getCartIndx(1) << " vote(s) " << setw(3) << avg << "%  ";
        getChart(cartAvg[i].getCartIndx(1));
        
        avg = (cartAvg[i].getCartIndx(2)/tRecFloat)*100;
        //cout << setw(18) << survey.item[i].getItem(3) << "  "
             //<< cartAvg[i].getCartIndx(2) << " vote(s) " << setw(3) << avg << "%  ";
        getChart(cartAvg[i].getCartIndx(2));
    }    
}
...
void Admin::deleteUsr(){
    ...
    usrArr[indx]->user.setCartSiz(0);       // Reset voteSiz
