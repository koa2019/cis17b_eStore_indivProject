...
void User::wrtCart(){
    ...
    outTxt << "cart[" ;
    for(int i=0; i < cartSiz; i++){    
        outTxt << cart.getCartIndx(i);
        if( !(i == (cartSiz-1))){ outTxt << ','; }
    }    
    outTxt << "]\n"; // Add 10 to charCount in reWrtTxt() for this array
            
    //outTxt << "NumCart: " << cart.getNumCart() << endl;
    //cout << "\nTotal number of cart: " << cart.getNumCart() << endl;
...
void User::wrtTxt(){
    ...
    outTxt<<"votSiz:  "; 
    outTxt<< (cartSiz/100) << (cartSiz/10%10) << (cartSiz%10) <<endl; 
    outTxt << "cart[";
...
void User::reWrtTxt(long begnFile){
    ...
    long cursor = (begnFile <= 0) ? 0 : (begnFile+charCount); // cartSiz is a int which is 4 bits  
    ...
    outTxt<<"votSiz:  "; 
    outTxt<< (getCartSiz()/100) << (getCartSiz()/10%10) << (getCartSiz()%10) <<endl;
    outTxt << "cart[";
    for(int i=0; i < NUMQQ; i++){    
        outTxt << cart.getCartIndx(i);
        if( !(i == (NUMQQ-1))){ outTxt << ','; }
...
void User::wrtBin(){
    ...
    // Write cartSiz to binary file
    outBin.write(reinterpret_cast<char *>(&cartSiz) , sizeof(int)); 
    
    // Write each element in Cart array
    for(int i=0; i < NUMQQ; i++){        
        int num = cart.getCartIndx(i);
...
void User::reWrtBin(long begnFile){
    ...
    // Rewrite cartSiz in binary file     
    outBin.write(reinterpret_cast<char *>(&cartSiz) , sizeof(int)); 
...
//******************************************************************************
//            Checks if current score is greater than cartSiz 
//******************************************************************************

bool User::isCartSiz(int score){
    
    if((score > 999 || score < 0)){ return false; }
    // cartSiz is formatted for max of 3 digits 
    
    else if(score > cartSiz ) { return true; }
...
void User::printUsr()const{  
    ...
    cout<<"\nCartSiz: " << cartSiz <<endl;
    cout<<"cart[";
    for(int i=0; i < NUMQQ; i++){    
        cout << cart.getCartIndx(i);
        if( !(i == (NUMQQ-1))){ cout<<','; }
    }    
    cout << "]\n"; // Add 10 to charCount in reWrtTxt() for this array
}
